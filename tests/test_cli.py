...
def test_package_version_matches_pyproject() -> None:
    ...
    assert __version__ == project["project"]["version"] == "0.20.0"
...
def test_language_server_paths_are_parsed(tmp_path: Path) -> None:
    arguments = build_parser().parse_args(
        ["--element-bundle-path", str(tmp_path / "bundle"), "--java-path", str(tmp_path / "java")]
    )

    assert arguments.element_bundle_path == tmp_path / "bundle"
    assert arguments.java_path == tmp_path / "java"


def test_console_and_ide_paths_are_parsed(tmp_path: Path) -> None:
    arguments = build_parser().parse_args(
        [
            "--console-config-path",
    ...
    assert arguments.console_config_path == tmp_path / "console.json"
    assert arguments.ide_settings_path == tmp_path / "settings.json"


def test_managed_actions_path_is_parsed(tmp_path: Path) -> None:
    arguments = build_parser().parse_args(["--actions-config-path", str(tmp_path / "actions.json")])

    assert arguments.actions_config_path == tmp_path / "actions.json"
